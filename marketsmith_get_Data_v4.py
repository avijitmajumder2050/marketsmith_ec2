...
def run():
    ...
    def stock_case(row):
        try:
            price_strength = float(row.get("Price Strength", 0))
            ...
            if float(row.get("EPS Strength", 0)) >= 80:
                matches += 1

            if str(row.get("Buyer Demand", "")).upper().startswith(("A", "B")):
                matches += 1

            if float(row.get("Group Rank", 100)) < 40:
                matches += 1
    ...
    df["Setup_Case"] = df.apply(stock_case, axis=1)
    ...
    print(f"\nFound {len(filtered_df)} qualified setups")
    ...
    s3.put_object(
        Bucket=BUCKET,
        Key="chartmaza-data/output/marketsmith_setups.csv",
